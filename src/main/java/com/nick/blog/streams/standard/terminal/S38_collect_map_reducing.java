...
public class S38_collect_map_reducing {
    public static void main(String... args) {
        ...
        System.out.println(map3);  //prints: {Chicago=[Person{name='Bob', city='Chicago', age=24}],
        ...
        Map<Boolean, Optional<Person>> map5 = list.stream()
                .collect(Collectors.partitioningBy(p -> "Denver".equals(p.getCity()), Collectors.reducing(BinaryOperator.maxBy(Comparator.comparing(Person::getAge)))));
        ...
        Person theOldest6 = list.stream()
                .collect(Collectors.reducing(new Person(), BinaryOperator.maxBy(Comparator.comparing(Person::getAge))));
        System.out.print(theOldest6);  //prints: Person{name='John', city='Denver', age=25}

        System.out.print("\n7.reduce(Identity, BinaryOperator)  : ");
        Person theOldest7 = list.stream().reduce(new Person(), BinaryOperator.maxBy(Comparator.comparing(Person::getAge)));
        System.out.print(theOldest7);  //prints: Person{name='John', city='Denver', age=25}
        ...
        System.out.print(map8);  //prints: {Chicago=Person{name='Bob', city='Chicago', age=24},
                                   //         Denver=Person{name='John', city='Denver', age=25},
                                   //         Boston=Person{name='Bill', city='Boston', age=25}}

        System.out.print("\n9.partitioningBy(isDenver, reducing(Identity, BinaryOperator): ");
        Map<Boolean, Person> map9 = list.stream()
                .collect(Collectors.partitioningBy(p -> "Denver".equals(p.getCity()), Collectors.reducing(new Person(), BinaryOperator.maxBy(Comparator.comparing(Person::getAge)))));
        ...
        int theOldestAge11 = list.stream().map(Person::getAge).reduce(0, (a1,a2)-> a1 > a2 ? a1 : a2);
        ...
        Map<Boolean, Integer> map13 = list.stream()
                .collect(Collectors.groupingBy(p -> "Denver".equals(p.getCity()), Collectors.reducing(0, Person::getAge, (a1,a2)-> a1 > a2 ? a1 : a2)));
